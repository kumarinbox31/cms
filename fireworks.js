...
// Prevent user clicks/taps from launching fireworks.
const DISABLE_CLICKS = false;
// Currently Chrome doesn't seem to like the `mix-blend-mode: lighten;` used on
// the <canvas> instances, so to allow a background to show through we have to
// disable the trailing effect on the firework stars. This toggle makes it easy
// to re-enable in the future.
const DISABLE_TRAILS = true;
// Prevents audio files from being downloaded and allows firework
// simulation to begin immediately.
const NO_AUDIO = true;

// Disabling IS_HEADER as it was only needed for my personal CodePen Profile.
