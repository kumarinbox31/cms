...
$pg_form_id = $this->input->get('pg_form_id', true);

if (empty($id) || empty($pg_form_id)) {
    // Set error message and redirect
    $ci->session->set_flashdata('error', 'Invalid request. Please try again.');
    redirect(base_url('admin/plugin/ab-payment-form?page=show-data&id=' . $pg_form_id));
    exit;
}

// Delete the record
$ins = $this->db->where('id', $id)->delete('ab_payment_data');
if(!$ins){
